<div id="content">
	<h1>Allgemein </h1>
	<p>
		Die Stadt Pappenheim ist ein staatlich anerkannter Luftkurort (4000 Einwohner, 410 m.) und liegt abseits von grossen Verkehrsstrassen 
		in einer Altmühlschleife zwischen Treuchtlingen und Eichstätt umgeben von herrlichen Mischwäldern. Viele historische Bauwerke der 
		ehemaligen Residenzstadt der Reichserbmarschälle zeugen  von einer bedeutenden Vergangenheit. Burg Pappenheim (1050), St.Galluskirche (9.Jhd.), 
		Augustiner Kloster (1372), ev. Stadtkirche(1476),  Altes Schloss (1588) und Judenfriedhof (12. Jhd.). 
	</p>

	<!-- Start genarating Gallery -->
	<section id="examples" class="examples-section">
		<div class="container">
			<div class="image-row">
				<div class="image-set">
					<?php 
					$xmlFile = './config/freizeit.xml'; 

						if (file_exists($xmlFile)) { 
						    $xml = simplexml_load_file($xmlFile); 

						    foreach ( $xml->image as $user )   
							{   
							  $text = $user->text;
							  $url = $user->url;	
							  $urlResize = $user->resize;
							  echo ' <a class="example-image-link" href= '.$url.' data-lightbox="example-set" data-title= '.$text.' ><img class="example-image" src= '.$urlResize.'  alt=""/></a>';
						    
							}   
							 
						} else { 
						    exit("Datei $xmlFile kann nicht geöffnet werden."); 
						} 
						?>
					</div>
				</div>
			</div>
		</section>
	<!-- End genarating Gallery -->

	<h1>Freibad</h1>
	<p>
		Das städtische, solarbeheizte Freibad bietet Badespaß für die ganze Familie. Auch für die ganz Kleinen wartet ein Planschbecken.
		Außerdem werden Schwimm- und Tauchkurse sowie Erlebnistage für Kinder angeboten.
		<a href="http://www.pappenheim.de/Freibad-Pappenheim.255.0.html" title="my website">Mehr</a> 
	</p>

	<h1>Steinbrüche</h1>
	<p>
		Rund um Pappenheim gibt es mehrere Touristen-Steinbrüche, die von Abenteuerlustigen besucht werden können.
		In Solnhofen, der Nachbargemeinde von Pappenheim, wurde beispielsweise der berühmte 
		<a href="http://de.wikipedia.org/wiki/Archaeopteryx" title="my website">Archeopteryx</a> 
		gefunden. Bei einem Ausflug in einen dieser Touristen-Steinbrüche finden Sie sicher eine kleine Versteinerung
		die Sie gerne mit nach Hause nehmen dürfen.
		<a href="http://www.pappenheim.de/Fossilien-in-Solnhofen.262.0.html" title="my website">Mehr</a> 
	</p>

	<h1>Radtouren</h1>
	<p>
		Die gut ausgebauten Radwege in Pappenheim und Umgebung laden zu einer gemütlichen Tour durch das Altmühltal ein. 
		Gerne stellen wir Ihnen Karten und beraten Sie über mögliche Ziele.	
		<a href="http://www.naturpark-altmuehltal.de/routen/altmuehltal-radweg-166/" title="my website">Mehr</a> 
	</p>

	<h1>Hochseilpark</h1>
	<p>
		Ja richtig, in Pappenheim gibt es seit einigen Jahren einen Klettergarten. Wagemutige können sich bei Kletterpfaden
		verschiedenster Schwierigkeitsstufen beweisen. 
		<a href="http://www.hochseilpark-pappenheim.de/" title="my website">Mehr</a> 
	</p>

	<h1>Weidenkirche</h1>
	<p>
		Die Weidenkirche besteht aus vielen Weidenruten die entlang eines Stahlgestelles zu einer offenen und natürlichen
		Kirche zusammenwachsen. 
		<a href="http://www.ejb.de/index.php?id=1205" title="my website">Mehr</a> 
	</p>

	<h1>Mal- und Töpferkurse</h1>
	<p>
		Unsere Pappenheimer Künstlerin Birgit van der Gang lädt Sie gerne in ihr Atelier ein. Im Urlaub Töpfern oder Malen,
		dass wäre doch was!
	</p>

	<h1>Burg Pappenheim</h1>
	<p>
		Romantische Burg zu Pappenheim mit neu angelegtem botanischem Garten, Naturkundemuseum und natürlich Folterkammer.
		<a href="http://www.grafschaft-pappenheim.de/burg-pappenheim.34.0.html" title="my website">Mehr</a> 
	</p>

	<h1>Eichstätt</h1>
	<p>
		Die Barock- und Bischofsstadt Eichstätt mit viel Kultur- und Freizeitangeboten.	
		<a href="http://www.eichstaett.de/kultur/" title="my website">Mehr</a> 
	</p>

	<h1>Weißenburg</h1>
	<p>
		Die Römerstadt Weißenburg mit den römischen Thermen und dem Römermuseum.
		<a href="https://www.weissenburg.de/kultur/" title="my website">Mehr</a> 
	</p>
</div>
